#!/bin/sh
# usage - sh fetchAdminlog.sh 2>&1 | tee  adminlogs$(date "+%y%m%d_%H%M%S").log >> /dev/null
...
do
    ...
    do
      OUTPUT="$(oc adm node-logs ${node} --path=kube-apiserver/${logfile} | jq 'select(.user.username == "admin")')"
